...
-- Contains 113 heroes, 9 heroes have been removed
...
heroesTest = {"npc_dota_hero_keeper_of_the_light","npc_dota_hero_monkey_king","npc_dota_hero_warlock","npc_dota_hero_rubick"}
...
function SendNextHeroesToClient( player, currentHero )
	local randomHero1 = heroesCustom[math.random( 1, #heroesCustom )]
	while randomHero1 == currentHero do
		randomHero1 = heroesCustom[math.random( 1, #heroesCustom )]
	end

	local randomHero2 = heroesCustom[math.random( 1, #heroesCustom )]
	while randomHero2 == currentHero or randomHero2 == randomHero1 do
		randomHero2 = heroesCustom[math.random( 1, #heroesCustom )]
	end

	local randomHero3 = heroesCustom[math.random( 1, #heroesCustom )]
	while randomHero3 == currentHero or randomHero3 == randomHero1 or randomHero3 == randomHero2 do
		randomHero3 = heroesCustom[math.random( 1, #heroesCustom )]
	end

	-- Store every player's possible respawn options
	local heroes = { randomHero1, randomHero2, randomHero3 }
	nextHeroOptions[player:GetPlayerID() + 1] = heroes

	-- Send hero options to player
	CustomGameEventManager:Send_ServerToPlayer( player, "next_hero_options", heroes )
end

function ReceiveNextHeroFromClient( eventSourceIndex, args )
	nextHeroSelection[EntIndexToHScript( eventSourceIndex ):GetPlayerID() + 1] = args["selection"]
end
		...
		if killedUnit:IsReincarnating() == false then
			...
			if killedUnit:GetTeam() == 2 then
				...
				if livesRadiant < 0 then
					...
					-- Send hero options to player (Radiant)
					SendNextHeroesToClient( killedUnit:GetPlayerOwner(), killedUnit:GetUnitName() )
					...
					elseif livesRadiant == 1 then
						...
						GameRules:SendCustomMessage( "<font color='#00FF00'>Radiant</font> has " .. livesRadiant .. " extra lives left.", 0, 0 )
					end
				end
			elseif killedUnit:GetTeam() == 3 then
				...
				if livesDire < 0 then
					...
					-- Send hero options to player (Dire)
					SendNextHeroesToClient( killedUnit:GetPlayerOwner(), killedUnit:GetUnitName() )
					...
					elseif livesDire == 1 then
						...
						GameRules:SendCustomMessage( "<font color='#FF0000'>Dire</font> has " .. livesDire .. " extra lives left.", 0, 0 )
					end
				end
			end
		end
...
-- Mana costs of items have been manually removed in npc_items_custom.txt
...
function CooldownPenalty( caster, ability, penaltyDuration, penaltyStarts, initialCooldown, cooldownPerStack )
	-- Apply/increment debuff
	local penalty = nil
	if not caster:HasModifier( "modifier_cooldown_penalty" ) then
		...
		penalty:SetDuration( penaltyDuration, true )
	else
	...
	-- Start putting the ability on cooldown after a certain number of stacks
	local penaltyStacks = penalty:GetStackCount()
	if penaltyStacks >= penaltyStarts then
		ability:StartCooldown( initialCooldown + ((penaltyStacks - penaltyStarts) * cooldownPerStack) )
	end
end
...
--			caster:GiveMana( 75 )
	...
	elseif eventInfo.abilityname ~= "earth_spirit_stone_caller" then
		...
		if ability ~= nil  then
			...
			-- Deal with Keeper of the Light
			if eventInfo.abilityname == "keeper_of_the_light_illuminate" then
				local linkedAbility = caster:FindAbilityByName( "keeper_of_the_light_spirit_form_illuminate" )
				if linkedAbility ~= nil then
					Timers:CreateTimer( {
						endTime = 0, callback = function()
							linkedAbility:EndCooldown()
							linkedAbility:RefreshCharges()
						end
					} )
				end
			end
			if eventInfo.abilityname == "keeper_of_the_light_spirit_form_illuminate" then
				local linkedAbility = caster:FindAbilityByName( "keeper_of_the_light_illuminate" )
				if linkedAbility ~= nil then
					Timers:CreateTimer( {
						endTime = 0, callback = function()
							linkedAbility:EndCooldown()
			...
			-- Drow Ranger cooldown penalty
			if eventInfo.abilityname == "drow_ranger_wave_of_silence" then
				CooldownPenalty( caster, ability, 10.0, 15, 0.0, 0.05 )
			end

			-- Lich & Storm Spirit & Windranger cooldown penalty
			if eventInfo.abilityname == "lich_sinister_gaze" or eventInfo.abilityname == "storm_spirit_electric_vortex" or eventInfo.abilityname == "windrunner_shackleshot" then
				CooldownPenalty( caster, ability, 10.0, 15, 0.0, 0.1 )
			end
			...
			if eventInfo.abilityname == "rubick_telekinesis" or eventInfo.abilityname == "shadow_demon_disruption" then
				CooldownPenalty( caster, ability, 10.0, 10, 0.0, 0.1 )
			end

			-- Morphling & Naga Siren cooldown penalty
			if eventInfo.abilityname == "morphling_adaptive_strike_str" or eventInfo.abilityname == "naga_siren_ensnare" then
				CooldownPenalty( caster, ability, 10.0, 10, 0.0, 0.2 )
			end

			-- Riki cooldown penalty
			if eventInfo.abilityname == "riki_poison_dart" then
				CooldownPenalty( caster, ability, 10.0, 5, 0.0, 0.2 )
			end

			-- Keeper of the Light cooldown penalty
			if eventInfo.abilityname == "keeper_of_the_light_will_o_wisp" then
				CooldownPenalty( caster, ability, 15.0, 10, 0.0, 0.05 )
			end

			-- Winter Wyvern cooldown penalty
			if eventInfo.abilityname == "winter_wyvern_winters_curse" then
				CooldownPenalty( caster, ability, 15.0, 10, 0.0, 0.2 )
			end

			-- Meepo cooldown penalty
			if eventInfo.abilityname == "meepo_earthbind" then
				CooldownPenalty( caster, ability, 15.0, 5, 0.0, 0.2 )
			end

			-- Faceless Void cooldown penalty
			if eventInfo.abilityname == "faceless_void_chronosphere" then
				CooldownPenalty( caster, ability, 15.0, 5, 0.0, 0.25 )
			end
		end
...
function OnNpcSpawn( eventInfo )
	...
	elseif spawnedUnit:IsRealHero() and not spawnedUnit:IsTempestDouble() then
		if spawnedUnit.HasDied == 1 then
			...
			-- Find out the name of the next hero
			local nextHero = nil
			if nextHeroSelection[player + 1] ~= 0 then
				nextHero = nextHeroOptions[player + 1][nextHeroSelection[player + 1]]
				nextHeroSelection[player + 1] = 0
			else
				CustomGameEventManager:Send_ServerToPlayer( spawnedUnit:GetPlayerOwner(), "hide_hero_options", nil )
				nextHero = nextHeroOptions[player + 1][math.random( 1, 3 )]
			end

			-- Precache the new hero to prevent a lag spike
			PrecacheUnitByNameAsync( nextHero, function()
				...
				local newHero = PlayerResource:ReplaceHeroWith( player, nextHero, 0, 0 )
...
function OnGameStateChange( eventInfo )
	...
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_PRE_GAME then
		local radiantMessage = "<font color='#00FF00'>Radiant</font> has " .. livesRadiant .. " extra lives left."
		local direMessage = "<font color='#FF0000'>Dire</font> has " .. livesDire .. " extra lives left."
...
function Activate()
	...
	CustomGameEventManager:RegisterListener( "next_hero_selection", ReceiveNextHeroFromClient )
end
